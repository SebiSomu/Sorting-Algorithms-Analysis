...
// Space: O(n)
// Divide-and-conquer: splits recursively then merges sorted halves.
...
// Time:  O(n log n) average  |  O(n²) worst (sorted input with last-element pivot)
...
// Space: O(log n) — explicit stack replaces call stack, avoids stack overflow
void quickSortIterative(std::vector<int>& data, Counter& cnt, int left, int right) {
    ...
    while (!stack.empty()) {
        ...
        if (l < r) {
            ...
            // Push smaller partition first to keep stack depth at O(log n)
            if (pi - l < r - pi) {
...
// Shell Sort
// Time:  O(n^(3/2)) with Knuth's gap  |  O(n log² n) with other sequences
// Space: O(1)
// Generalization of Insertion Sort using decreasing gap intervals.
...
// Time:  O(n²) average/worst  |  O(n) best (already sorted)
// Space: O(1)
// Alternates direction each pass; handles turtles and rabbits better than Bubble Sort.
...
// Space: O(1)
// Improves Bubble Sort using a shrinking gap (shrink factor ≈ 1.3).
...
// Space: O(range)
// Places each element into its corresponding hole; optimal when range ≈ n.
...
// Strategy behind std::sort in most standard library implementations.
...
// Space: O(n)
// Hybrid of Merge Sort and Insertion Sort; used in Python and Java standard libraries.
void timSort(std::vector<int>& data, Counter& cnt) {
    ...
    // Merge sorted runs bottom-up, doubling merge size each iteration
...
// Time:  O(n log² n) — best, average, worst
// Space: O(log² n)
// Comparison network sort; highly parallelizable on GPU/hardware.
// Requires n to be a power of 2; falls back to std::sort otherwise.
